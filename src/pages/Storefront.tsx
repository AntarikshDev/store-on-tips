...
const Storefront = () => {
  ...
  const renderSection = (section: any, index: number) => {
    switch (section.type) {
      case 'hero':
        return (
          <section key={index} className="text-center py-12 md:py-16 px-4 relative overflow-hidden" style={{ backgroundColor: colors.secondary }}>
            {(section.image || store.banner_url) && (
              <img src={section.image || store.banner_url} alt="" className="absolute inset-0 w-full h-full object-cover opacity-20" />
            )}
            <div className="relative z-10">
              <h1 className="text-2xl md:text-4xl font-bold mb-3" style={{ fontFamily: fonts.heading }}>
                {section.title || store.description || `Welcome to ${store.name}`}
              </h1>
              {section.subtitle && <p className="text-sm opacity-60 mb-6 max-w-md mx-auto">{section.subtitle}</p>}
